...
begin in June, 2024. The research will focus on three
...
MA to the north.The county is situated on Buzzard's Bay to the south.
...
Equine Encephalitis (EEE). AWC trees have large roots that form "crypts"
that hold water throughout the year. These crypts serve as a main
habitat for MEL larval growth.
...
The mosquito surveillance dataset encompasses counts by species from all mosquito traps deployed in Bristol County, MA, from 2007 to 2023. The BCMCP, perform mosquito surveillance from June through October annually. During this interval, CDC light traps baited with carbon dioxide are placed at various sentinel sites each week. Some of the sentinel site have been used for over 50 years. The traps are installed in the afternoon and retrieved the following morning. Designed to attract mosquitoes in search of blood meals, light traps serve as a reliable measure of the actively feeding mosquito population, thereby establishing them as the standard method for EEE surveillance.
...
The  In addition to mosquito
counts by species, the dataset includes the town or city in which they
were collected, the collection data, and whether the mosquitoes were
submitted for virus testing.

While the surveillance data provide insight into mosquito population
estimates and changes in species composition, it is missing one key
feature. The virus isolation results are not included in this dataset.
However, virus isolations in mosquitoes are published on the MDPH
website by town/city and collection date. With this information, one can
identify the trap event contained in the dataset which allow for
calculations of key measure, such as minimum infection rate (MIR).
...
This dataset will be integrated with other datasets using spatial or
temporal attributes. These datasets will include biological and
...
The geographic focus for this study is Bristol County. One factor to
consider is if early virus isolations are associated with cases in
