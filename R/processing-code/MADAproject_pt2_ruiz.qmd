...
wx_selected <- wx %>%
  select(STATION, NAME, DATE, PRCP, TMAX, week_num)
...
#### The mosquito dataset spans 2007 to 2023. However, the virus and case data only covers the years 2012 to 2020. We will create two datasets One that filters the dates to cover the virus and case data. And one with all records. 
...
mosquito_summarized <- mosquito_modified %>%
  group_by(`collection date`, week_num, town, `trap type`, `species code`, mcd, `submitted for testing`) %>%
  summarise(pool_size_sum = sum(`pool size`, na.rm = TRUE), .groups = 'drop')
...
wide_data <- mosquito_summarized %>%
  pivot_wider(
    ...
    id_cols = c(`collection date`, week_num, town, `trap type`, mcd)
  ) %>%
  # Now use replace_na() to fill in NAs, specifying columns dynamically or by name after checking what columns are created
  replace_na(list(your_dynamic_or_specific_column_names_here = 0))  
...
filtered_mosquito_data <- wide_data %>%
  filter(`collection date` >= as.Date("2011-01-01") & `collection date` <= as.Date("2022-12-31"))
